...
static const int GRID_X_SIZE = 160;
static const int GRID_Y_SIZE = 80;
...
int main(int argc, char const *argv[])
{
    ...
    FluidSolverMemoryPool mem_pool(GRID_X_SIZE, GRID_Y_SIZE, float(WORLD_X_SIZE) / GRID_X_SIZE, float(WORLD_Y_SIZE) / GRID_Y_SIZE);
    fluid_solver.initialize(mem_pool);
	Renderer renderer(0,0, WORLD_X_SIZE, WORLD_Y_SIZE);
	Canvas canvas(GRID_X_SIZE * 5, GRID_Y_SIZE * 5);

	// Setup
    fluid_domain.addFluidSource(FluidSource(2.0 / GRID_X_SIZE, 0.35, 2.0 / GRID_Y_SIZE, 1 - 2.0 / GRID_Y_SIZE, 0.0, 0.0, 0.0, 1));
    ...
    fluid_domain.addFluidSource(FluidSource(0.8, 1.2, 0.1, 0.5, 0.0, 4.0, 2.0, 2));
